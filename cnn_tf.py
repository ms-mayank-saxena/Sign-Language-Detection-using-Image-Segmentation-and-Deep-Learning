...
def cnn_model_fn(features, labels, mode):
    input_layer = tf.reshape(
        features["x"], [-1, image_x, image_y, 1], name="input")
    ...
    print(flat.shape)
    dense = tf.layers.dense(inputs=flat, units=128,
    ...
    logits = tf.layers.dense(
        inputs=dropout, units=num_of_classes, name="logits")
    ...
    predictions = {"classes": tf.argmax(
        input=logits, axis=1), "probabilities": tf.nn.softmax(logits, name="softmax_tensor")}
    ...
    onehot_labels = tf.one_hot(indices=tf.cast(
        labels, tf.int32), depth=num_of_classes)
